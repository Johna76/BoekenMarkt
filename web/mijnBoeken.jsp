...
<div id="templatemo_main">
   	<h1>Mijn Boeken</h1>         
        <div id="list">
        <h3>Lijst van al uw aangeboden boeken</h3> 
        <table width="900" id="OverviewList">
        	<tr>
        		...
        		<th></th>
                    ...
                    <% List<AangebodenBoek> mijnBoeken = (List<AangebodenBoek>)session.getAttribute("mijnBoeken");                    
                    for (AangebodenBoek boek : mijnBoeken) {
                        String datum = DateFormat.getDateInstance(DateFormat.DEFAULT).format(boek.getDatum());
                        %>
                        <tr>
                        <td><%= boek.getBoekID().getTitel()%></td>
        		<td><%= boek.getConditie()%></td>
        		<td><%= boek.getPrijs()%> EUR</td>
        		<td><%= datum%></td> 
			<td><a href="/Web_BoekenMarkt/DetailMijnBoekServlet?BoekID=<%= boek.getId()%>" class="more">Select</a></td>
        	</tr>
                <%
                    }
                %>                           
